...
namespace RepositoryContract.Demo.Tests {
  ...
  public class AutoValueManagerTests {
    ...
    public void ApplyValuesOnAdd_InitializesHighestValueOnlyOncePerScope() {
      ...
      int getHighestExistingValueCalls = 0;

      AutoValueEntity firstEntity = new AutoValueEntity();
      manager.ApplyValuesOnAdd(
        firstEntity,
        propertyInfo => {
          getHighestExistingValueCalls++;
          return 7;
        },
        scopeKey
      );
      ...
      manager.ApplyValuesOnAdd(
        ...
        propertyInfo => {
          getHighestExistingValueCalls++;
          throw new InvalidOperationException("Highest value lookup should not happen after the initial cache population.");
        },
        scopeKey
      );

      Assert.AreEqual(9, secondEntity.Number);
      Assert.AreEqual(1, getHighestExistingValueCalls);
    }
